...
The previous two examples make use of the `animated` prop. This prop allows you to use the `transition` and `animation` properties from display: none or hidden. In the future, there will be support for transitioning display, but this should do the trick in the meantime.

If you're curious about how the accordion is animated, you can use a built-in Qwik UI variable called `--qwikui-collapsible-content-height`.
