...
> Maybe the items are out of order and you want to sort them? Programmatically update them directly in the JSX? You can do that!

### Object example

<Showcase name="object" />

> In a case like the above example, where the display value can get cluttered, you may want to provide a `value` prop to the `<Select.Item />` component. We will refer to this in the next section.
...
To use the select in a form, we provide a visually hidden native select element inside of `<Select.HiddenSelect>`.
...
Something to be aware of, is the `<Select.DisplayText />` component will now display whatever you put inside of it.
...
We can provide a custom placeholder to the `<Select.DisplayText />` component by adding the `placeholder` prop.

<Showcase name="hero" />

When a value is not selected, the placeholder is displayed.

> The `placeholder` prop is only used when the `multiple` prop is set to `false`.
...
`Select`, `Select.Trigger`, `Select.DisplayText`, `Select.Popover`, and `Select.Listbox` all have data attributes that are used to track state.
