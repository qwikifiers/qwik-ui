...
Reveals a list of items to choose from, often triggered by a button.
...
<FeatureList
  features={[
    ...
    'Automatic focus management for first and last items',
...
<AnatomyTable
  propDescriptors={[
    {
      name: 'Select.Label',
      description: 'A label for the Select.Group.',
    },
    {
      name: 'Select.Root',
    ...
    {
      name: 'Select.Listbox',
      description: 'An unordered list that displays selectable items.',
    },
    {
      name: 'Select.Item',
      description: 'A selectable item within the SelectListbox.',
    },
    {
...
By default, the content inside of the `<Select.ItemLabel />` component is the option value.
...
We can add an indicator to the selected option by adding the `<SelectIndicator />` component to the `<SelectOption />` component.

This is useful for indicating the selected option in a list of options.

### Multiple selections

Sometimes we want to allow the user to select multiple pre-defined options. We can do that by setting the `multiple` prop to `true`.

<Showcase name="multiple" />

Something to be aware of, is the `<SelectValue />` component will now display whatever you put inside of it.

This can be used in unison with the `bind:value` prop for more customization. For example, a pill component that displays the selected values.
...
<KeyboardInteractionTable
  keyDescriptors={[
    {
      keyTitle: 'Space',
      description: `Toggles the selection of the highlighted option without closing the menu.`,
    },
    {
      keyTitle: 'Enter',
      description: `Toggles the selection of the highlighted option without closing the menu.`,
