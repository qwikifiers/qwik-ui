...
To show UI when there are no items in the listbox, use the `Empty` component.

<Showcase name="empty" />

If this component is not used, the listbox will be closed by default when there are no visible items.
