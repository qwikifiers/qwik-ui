---
title: Qwik UI | Pagination
---

import { statusByComponent } from '~/_state/component-statuses';

import { ShowcaseHero, ShowcaseStyling, ShowcaseCustomButtonLabels, ShowcaseCustomArrows, ShowcaseHidePrevNextButtons, ShowcaseDisabled } from './showcase-components';

<StatusBanner status={statusByComponent.headless.Pagination} />

# Pagination

<ShowcaseHero />

## ✨ Features

<FeatureList
  features={[
    'Pagination ranges',
    'Custom Arrows',
    'Custom Button Labels',
    'Fully Customizable',
  ]}
/>

<Note status="warning">
  The Pagination component is about to undergo a **major refactor**. The API and behavior will
  change drastically in the coming weeks.

  <div>You can expect it to match the conventions, developer
  experience, and performance of the other components.</div>
</Note>

## Hero Example

<ShowcaseHero />

## Styling

<ShowcaseStyling />

## Custom Button Labels

<ShowcaseCustomButtonLabels />

## Custom Arrows

<ShowcaseCustomArrows />

## Hide Next/Prev Buttons

<ShowcaseHidePrevNextButtons />

## Disabled

<ShowcaseDisabled />

## API

The Pagination component API provides a set of properties that allow you to customize the behavior and appearance of the pagination.

Here are the notable properties for this component:

<APITable
  propDescriptors={[
    {
      name: 'selectedPage',
      type: 'number',
      description: 'Current Page Index (start from 1)',
    },
    {
      name: 'totalPages',
      type: 'number',
      description: 'The total of available pages',
    },
    {
      name: 'onPageChange$',
      type: 'function',
      description: `Emitted when a button is clicked`,
    },
    {
      name: 'siblingCount',
      type: 'number',
      description: `set sibling items close to the selected value`,
    },
    /*
   {
      name: 'boundaryCount',
      type: 'number',
      description: `set adjacent items to the start and end page number`,
    },
    */
    {
      name: 'hidePrevButton',
      type: 'boolean',
      description: `hide the previous button`,
    },
    {
      name: 'hideNextButton',
      type: 'boolean',
      description: `hide the next button`,
    },
    {
      name: 'disabled',
      type: 'boolean',
      description: `disable mouse events`,
    },
    {
      name: 'customArrowTexts',
      type: 'ArrowLabels',
      description: `customize the PREVIOUS and NEXT button labels `,
    },
    {
      name: 'gap',
      type: 'string',
      description: `set the gap between items (i.e. '10px', '1rem', ...)`,
    },
    {
      name: 'class',
      type: 'string',
      description: `custom class for pagination wrapper `,
    },
    {
      name: 'defaultClass',
      type: 'string',
      description: `custom class for each pagination item`,
    },
    {
      name: 'selectedClass',
      type: 'string',
      description: `custom class for the selected item`,
    },
    {
      name: 'dividerClass',
      type: 'string',
      description: `custom class for the divider item`,
    },
    {
      name: 'prevButtonClass',
      type: 'string',
      description: `custom class for the Next button`,
    },
    {
      name: 'nextButtonClass',
      type: 'string',
      description: `custom class for the Next button`,
    },
  ]}
/>
