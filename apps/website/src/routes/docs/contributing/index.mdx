...
Qwik components execute based on interactions, each interactive piece is a possible entry point of the application. This means that Qwik components can render asynchronously and even out of order. Manu Almeida covers this in-depth on the [builder.io blog](https://www.builder.io/blog/hydration-tree-resumability-map).

While this offers substantial benefits, this poses a challenge when you need to maintain a specific order or index for child components when in a CSR environment.
...
Here's an example of how an inline component might be used in a breadcrumb implementation:
